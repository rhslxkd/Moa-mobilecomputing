    ...
    <Animated.View style={[skeletonStyles.row, { opacity }]}>
      ...
      <View style={{ flex: 1, gap: 8 }}>
        ...
        <View style={skeletonStyles.topRow}>
          <View style={[skeletonStyles.bar, { width: "45%", height: 14 }]} />
          <View style={[skeletonStyles.bar, { width: "18%", height: 12 }]} />
        </View>
        {/* 마지막 메시지 */}
        <View style={[skeletonStyles.bar, { width: "70%", height: 12 }]} />
      </View>
    </Animated.View>
  );
}
...
function KakaoGroupAvatar({ memberCount, bgCard }: { memberCount: number, bgCard: string }) {
  ...
  if (count <= 1) return <DummyAvatar size={size} color={BG_COLORS[0]} radius={20} />;
  if (count === 2) {
    const s = 30;
    return (
...
export default function ChatScreen() {
  ...
  const loadRooms = useCallback(() => {
    setIsLoading(true);
    ChatAPI.rooms()
      .then((ds) => setRooms(ds.map(dtoToRoom)))
      .catch(() => {})
          ...
          {FILTER_TABS.map((tab) => {
            ...
            return (
              <TouchableOpacity
                key={tab.id}
                onPress={() => {
                  if (tab.id === 'gemini') Linking.openURL('https://gemini.google.com');
                  else setFilter(tab.id);
                }}
                activeOpacity={0.8}
                style={[styles.filterPill, { backgroundColor: isSelected ? C.text : C.bgCard, borderColor: isSelected ? C.text : C.border }]}
              >
                {tab.id === 'gemini' && <View style={{ marginRight: 4 }}><GeminiSpark size={14} color={isSelected ? C.bgCard : C.textSub} /></View>}
                {tab.id === 'unread' && <View style={isSelected ? { marginRight: 4 } : {}}><UnreadChatIcon size={16} strokeColor={isSelected ? C.text : C.bgCard} /></View>}
                {(tab.id !== 'unread' || isSelected) && (
                  <Text style={[styles.filterPillText, { color: isSelected ? C.bgCard : C.textSub }]}>{tab.label}</Text>
                )}
              </TouchableOpacity>
            );
          })}
...
const styles = StyleSheet.create({
  ...
  filterPill: { flexDirection: "row", alignItems: "center", justifyContent: "center", paddingHorizontal: 14, paddingVertical: 8, borderRadius: 20, borderWidth: 1 },
  filterPillText: { fontSize: 14, fontWeight: "600" },
  roomItem: { flexDirection: "row", alignItems: "center", paddingHorizontal: 20, paddingVertical: 14 },
