  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: C.bg }} edges={['top']}>
      ...
      <ScrollView style={{ flex: 1 }} showsVerticalScrollIndicator={false}>
        ...
        {/* 미디어 카드 */}
        <View style={[styles.card, { backgroundColor: C.bgCard, shadowColor: C.text }]}>
          <TouchableOpacity
          ...
          >
            <View style={styles.itemTitleRow}>
              ...
              <Text style={[styles.itemTitle, { color: C.text }]}>사진/동영상</Text>
            </View>
          </TouchableOpacity>

          <MenuListItem icon="file"     label="파일"        color="#7F8C8D" textColor={C.text} onPress={() => router.push({ pathname: '/(screens)/chat/ChatArchive', params: { projectId: roomId, initialTab: 'file' } })} />
          <MenuListItem icon="link"     label="링크"        color="#007AFF" textColor={C.text} onPress={() => router.push({ pathname: '/(screens)/chat/ChatArchive', params: { projectId: roomId, initialTab: 'link' } })} />
          <MenuListItem icon="calendar" label="전체 To-Do"  color="#4FC3F7" textColor={C.text} onPress={() => router.push("/(tabs)/todos")} />
        </View>

        {/* 기능 카드 (그리드) */}
        <View style={[styles.card, { backgroundColor: C.bgCard, shadowColor: C.text }]}>
          <View style={styles.gridRow}>
            <GridItem icon="announcement" label="공지" color="#007AFF" textColor={C.text} bgColor={C.bgMuted} onPress={() => setNoticeOpen(true)} />
            <GridItem icon="vote"         label="투표" color="#9B59B6" textColor={C.text} bgColor={C.bgMuted} onPress={() => setPollOpen(true)} />
          </View>
        </View>

        {/* 멤버 카드 */}
        <View style={[styles.card, { backgroundColor: C.bgCard, shadowColor: C.text, marginBottom: Math.max(insets.bottom, 40) }]}>
          <Text style={[styles.memberCountText, { color: C.textSub }]}>대화상대 {members.length}</Text>

          {members.map(m => (
            <View key={m.id} style={styles.memberRow}>
              <View style={[styles.smallAvatar, { backgroundColor: m.me ? C.bgMuted : '#A5F3FC' }]}>
                <Text style={[styles.avatarInitial, { color: C.text }]}>{m.initial}</Text>
              </View>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                {m.me && (
                  <View style={[styles.selfBadge, { backgroundColor: C.textMuted }]}>
...
function MenuListItem({ icon, label, color, textColor, onPress }: { icon: any, label: string, color: string, textColor: string, onPress: () => void }) {
  return (
...
function GridItem({ icon, label, color, textColor, bgColor, onPress }: { icon: any, label: string, color: string, textColor: string, bgColor: string, onPress?: () => void }) {
  return (
    <TouchableOpacity style={[styles.gridItem, { backgroundColor: bgColor }]} onPress={onPress}>
